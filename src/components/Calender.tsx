...
import { endOfMonth, getDate, getDaysInMonth, isToday, startOfMonth } from 'date-fns'
...
const Calender: FC<ICalender> = (props): ReactElement => {
  ...
  const day_of_month = getDate(date)
    ...
    <section className="my-8">
      <div className="grid grid-cols-7 items-center justify-items-center text-center shadow-lg bg-purple-100 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20 border border-gray-100">
        ...
        {Array.from({ length: number_of_days }).map((_, idx) => {
          const date = idx + 1
          const isToday = date === day_of_month

          return <Cell key={idx} className={`w-full aspect-square grid place-content-center hover:bg-clip-padding rounded-sm cursor-pointer ${is_Today && isToday ? "bg-gradient-to-r from-fuchsia-600 to-pink-600" : "hover:backdrop-filter hover:backdrop-blur-sm hover:bg-opacity-50"}`}>
            {date}
          </Cell>
        })}
